...
private:
    ...
    void CreateShaders() {
        ...
        // Ïèêñåëüíûé øåéäåð ñ ÐÀÑÊÐÀÑÊÎÉ ÊÀÑÊÀÄÎÂ
        const char* psCode = R"(
            ...
            float4 PSMain(VSOutput input) : SV_TARGET {
                ...
                float3 cascadeColor = float3(1, 1, 1); // Áåëûé ïî óìîë÷àíèþ
                ...
                if (useShadow != 0) {
                    ...
                    if (depth <= cascadeSplit0) {
                        ...
                        cascadeIndex = 0;
                        cascadeColor = float3(1, 0.2f, 0.2f); // Êðàñíûé - êàñêàä 0 (áëèæíèé)
                    } else if (depth <= cascadeSplit1) {
                        shadowPos = input.shadowPos1;
                        cascadeIndex = 1;
                        cascadeColor = float3(0.2f, 1, 0.2f); // Çåëåíûé - êàñêàä 1
                    } else if (depth <= cascadeSplit2) {
                        shadowPos = input.shadowPos2;
                        cascadeIndex = 2;
                        cascadeColor = float3(0.2f, 0.2f, 1); // Ñèíèé - êàñêàä 2
                    } else {
                        shadowPos = input.shadowPos3;
                        cascadeIndex = 3;
                        cascadeColor = float3(1, 1, 0.2f); // Æåëòûé - êàñêàä 3 (äàëüíèé)
                    }
                ...
                // Íàêëàäûâàåì öâåò êàñêàäà íà òåíè (óìíîæàåì, ÷òîáû íå ïåðåñâå÷èâàòü)
...
public:
    ...
    void Draw() override {
        ...
        psCB.useShadow = (game->CSMShadowMapSRVs[0] != nullptr) ? 1 : 0;
